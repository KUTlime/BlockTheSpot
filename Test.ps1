...
1..10 | ForEach-Object {
    Write-Host "Buffer size $_ MB: $((Measure-Command {
        Get-File -Uri $uri -TargetFile "Test$_.exe" -BufferSize $_
    }).TotalMilliseconds) s"
}
Write-Host "Benchmark (Start-BitsTransfer) $((Measure-Command {Start-BitsTransfer -Source $uri -Destination 'Test.exe'}).TotalMilliseconds) s"
Write-Host "Benchmark (WebClient) $((Measure-Command { (New-Object ([System.Net.WebClient])).DownloadFile($uri, 'Test.exe')}).TotalMilliseconds) s"